...
public class WeatherSensorConsumer implements AutoCloseable {
    ...
    private void consumeWeatherInfo(ConsumerRecord<Geolocation, WeatherInfo> weatherInfo) {
        ...
        if (Objects.isNull(weatherInfo.key())) {
            ...
            WeatherAggregate aggre = cache.compute(weatherInfo.key(), (key, value) -> {
                if (Objects.isNull(value)) {
                    ...
                    return new WeatherAggregate(Math.max(value.getMaxTemperature(),
                                                         weatherInfo.value().getTemperature()),
                                                Math.min(value.getMinTemperature(),
                                                         weatherInfo.value().getTemperature()),
    ...
    public static void main(String[] args) {
        ...
        properties.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "kafka-0:9092,kafka-1:9094,kafka-2:9096");
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, "aggregador");
